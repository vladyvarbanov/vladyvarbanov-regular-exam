...
suite('Add Products page', function() {
  test('Page title', async function() {
    let res = await fetch("http://127.0.0.1:8888/add-product");
    let body = await res.text();
    assert.ok(body.includes("<h1>Add New Product</h1>"));
  });

  test('Product HTML form', async function() {
    let res = await fetch("http://127.0.0.1:8081/Add-Product");
    let body = await res.text();
  ...
  test('Add valid product', async function() {
    let res = await fetch(
      "http://127.0.0.1:8888/Add-Product",
      {
        method: 'POST',
        headers: {
  ...
  test('Add invalid product', async function() {
    ...
    res = await fetch("http://127.0.0.1:8888/");
    body = await res.text();
